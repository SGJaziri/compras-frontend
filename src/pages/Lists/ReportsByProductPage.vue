...
/* multiselección productos */
type ProdOpt = { id: ID; name: string };
const selectedProductIds = ref<ID[]>([]);
...
function extractItems(json: any): ReportItem[] {
  ...
  if (json?.items)   return json.items.map(safeMapFlat).filter(Boolean) as ReportItem[];
  if (json?.results) return json.results.map(safeMapFlat).filter(Boolean) as ReportItem[];
  if (json?.data)    return json.data.map(safeMapFlat).filter(Boolean) as ReportItem[];
  if (json?.restaurants) {
...
async function fetchData(){
  ...
  } catch(e:any){ error.value = e?.message ?? "Error al cargar reportes"; rawData.value = []; }
  finally{ loading.value = false; }
}

/* opciones productos (respaldo por nombre si falta id) */
const productOptions = computed<ProdOpt[]>(() => {
  const m = new Map<string, string>(); // key estable
...
const filteredItems = computed(() => {
  if (selectedProductIds.value.length === 0) return items.value;
...
async function openPDF() {
  try {
    ...
    const filename =
      `reporte_productos_${start.value}_a_${end.value}` +
...
<template>
  <div class="p-6">
    <h1 class="text-2xl font-semibold mb-4">Reportes por producto</h1>
    ...
    <!-- Badges informativos cuando hay selección -->
    <div v-if="showBadges" class="mb-3 flex flex-wrap gap-2">
      <span class="px-3 py-1 rounded-full border bg-indigo-50 border-indigo-200 font-semibold">
        Cantidad total: {{ totalQtySel }} {{ unitSummary }}
      </span>
      <span v-if="unitPriceHomogeneous !== null" class="px-3 py-1 rounded-full border bg-green-50 border-green-200 font-semibold">
        Precio unit.: S/ {{ unitPriceHomogeneous.toFixed(2) }}
      </span>
    </div>
