...
/** << NUEVO >>: Init ancho que permite body como objeto */
export type ApiInit = Omit<RequestInit, 'body'> & {
  body?: any; // normalizeBody lo serializa si hace falta
};
...
function buildHeaders(init?: ApiInit): Headers {
  ...
  if (token && !h.has('Authorization')) h.set('Authorization', `Token ${token}`);
  if (!h.has('Accept')) h.set('Accept', 'application/json, text/plain, */*');

  const body = (init as any)?.body;
  const isFormData = (typeof FormData !== 'undefined') && body instanceof FormData;
  if (!isFormData && body != null && !h.has('Content-Type')) h.set('Content-Type', 'application/json');

  return h;
}

/** Normaliza el body: si es objeto y no es FormData, lo serializa a JSON */
function normalizeBody(init?: ApiInit): BodyInit | undefined {
  const body = (init as any)?.body;
  ...
  try { return JSON.stringify(body); } catch { return body as BodyInit; }
}
      ...
      if (ct.includes('application/json')) {
        ...
        if (j?.detail && typeof j.detail === 'string') {
          ...
          for (const k of Object.keys(j)) {
            const v = (j as any)[k];
            if (Array.isArray(v) && v.length) { message = `${k}: ${v[0]}`; break; }
            if (typeof v === 'string')        { message = `${k}: ${v}`;  break; }
          }
          if (message.startsWith('HTTP')) message = JSON.stringify(j);
        }
      } else {
...
export function openPdfUrl(href: string) {
